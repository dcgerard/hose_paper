...
% In particular, tensor decompositions \citep{kolda2009tensor}, as a way of identifying tensor structure, are being applied to each of these fields.
...
\begin{figure*}
\begin{center}
\includegraphics{./fig/mario_mean_gif}
...
\begin{figure*}
\begin{center}
\includegraphics{./fig/mario_noise_gif}
...
\begin{figure}
\begin{center}
%%\includegraphics{low_multilinear_rank}
\includegraphics{./fig/low_mario_rank}
\caption{Mode specific singular values of the Mario movie of Figure \ref{fig:mario.movie}.}
\label{fig:example.svs}
\end{center}
%\caption{Mode specific singular values of a tensor that has full mode 1 rank but low mode 2 and mode 3 rank (left), and the mode specific singular values of the Mario movie of Figure \ref{fig:mario.movie} (right).}
\end{figure}

%%In general, having low rank along one mode does not imply having low rank along all modes. It is possible to have different mode-specific ranks, and possible to shrink toward low multilinear rank. For example, in the left plot of Figure \ref{fig:mario.movie} we simulated a $10 \times 10 \times 10$ dimensional tensor data set that contains full rank along the first mode and low rank along modes 2 and 3. If we were only to observe the first mode matrix unfolding, then we would not be aware of the low rank structure, when in fact there is extremely low multilinear rank structure.
